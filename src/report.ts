...
CREATE TABLE lineage (run_id TEXT, parent_run_id TEXT, attempt_mode TEXT);`);
  ...
  for (const experiment of experimentMetadata.values()) for (const plan of experiment.reporting.comparisons) for (const candidate of plan.candidate_configuration_ids) {
    const paired = initial.filter(row => row.experiment === `${experiment.id}@${experiment.version}` && row.configuration_id === plan.baseline_configuration_id && row.evaluation.eligible_for_quality_aggregate).flatMap(left => initial.filter(right => right.experiment === left.experiment && right.configuration_id === candidate && right.evaluation.eligible_for_quality_aggregate && right.task === left.task && right.compatible_key === left.compatible_key).map(right => ({ task: left.task, baseline: plan.baseline_configuration_id, candidate, score_delta: right.evaluation.artifact_quality_score - left.evaluation.artifact_quality_score })));
    comparisons.push({ id: plan.id, experiment: `${experiment.id}@${experiment.version}`, baseline: plan.baseline_configuration_id, candidate, paired_task_deltas: paired, aggregate_score_delta: paired.length ? paired.reduce((total, item) => total + item.score_delta, 0) / paired.length : null, qualification: paired.length ? "declared candidate-minus-baseline paired task deltas; smoke only" : "declared comparison has no compatible scorable paired task observations" });
  }
  const task_category_language = initial.map(row => ({ task: row.task, category: row.category, languages: row.languages, configuration_id: row.configuration_id, score: row.evaluation.artifact_quality_score ?? null, passed: row.evaluation.end_to_end_passed === true, quality_eligible: row.evaluation.eligible_for_quality_aggregate === true }));
  const effort_quality = initial.map(row => ({ configuration_id: row.configuration_id, effort: row.effort, score: row.evaluation.artifact_quality_score ?? null, timing: row.metrics.timing?.status === "complete" ? row.metrics.timing.wall_time_ms : null, tokens: row.metrics.tokens?.status === "complete" ? row.metrics.tokens.total : null, cost: row.metrics.cost?.status === "complete" ? row.metrics.cost.amount ?? null : null, qualification: "missing metrics are null, never zero" }));
  const error_details = initial.flatMap(row => row.errors.map((error: any) => ({ configuration_id: row.configuration_id, terminal_reason: row.terminal.reason, attribution: error.attribution ?? row.terminal.attribution, phase: error.phase ?? "unknown", code: error.code ?? "unknown", message: error.message ?? "" })));
  const resource_observations = initial.map(row => ({ configuration_id: row.configuration_id, status: row.metrics.resources?.status ?? "unavailable", observations: row.metrics.resources ?? null }));
  const data = { report_data_version: REPORT_DATA_VERSION, generated_from: "immutable results/ source artifacts", warnings: ["Smoke evidence has one repetition and is not suitable for confidence intervals or ranking.", "Retries/resumes are visible but excluded from v0 headline aggregates.", "Unavailable telemetry is missing, not zero."], campaigns: [...new Set(rows.map(row => row.campaign_id))].sort(), configurations: configs, comparisons, operational_failures: initial.filter(row => !row.terminal.operational_success).map(row => ({ configuration_id: row.configuration_id, terminal_reason: row.terminal.reason, attribution: row.terminal.attribution, errors: row.evaluation.reason ?? "no evaluation" })), error_details, task_category_language, effort_quality, resource_observations, history: rows.map(row => ({ observed_at: row.observed_at, campaign_id: row.campaign_id, configuration_id: row.configuration_id, task: row.task, score: row.evaluation.artifact_quality_score ?? null, compatibility: "qualified: model snapshot unavailable and local topology" })), ingestion_errors: errors, lineage: rows.filter(row => row.attempt.number > 1).map(row => ({ configuration_id: row.configuration_id, task: row.task, attempt: row.attempt })) };
  const dataPath = join(output, "report-data.json"); await writeFile(dataPath, `${JSON.stringify(data, null, 2)}\n`);
  const html = `<!doctype html><meta charset="utf-8"><title>Agent Harness Test report</title><style>body{font:16px system-ui;margin:2rem;max-width:1100px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #bbb;padding:.45rem;text-align:left}.warn{background:#fff4cc;padding:1rem}</style><h1>Agent Harness Test report</h1><p class="warn">${safe(data.warnings.join(" "))}</p><h2>Configuration metrics</h2><table><tr><th>Configuration</th><th>Headline score (quality denominator)</th><th>End-to-end pass</th><th>Operational completion</th><th>Invalid/infrastructure</th></tr>${configs.map(c => `<tr><td>${safe(c.configuration_id)}</td><td>${c.headline_score ?? "unavailable"} (${c.quality_denominator})</td><td>${c.end_to_end_pass_rate ?? "unavailable"}</td><td>${c.operational_completion_rate ?? "unavailable"}</td><td>${c.invalid_or_infrastructure_attempts}</td></tr>`).join("")}</table><h2>Paired task deltas</h2><pre>${safe(JSON.stringify(comparisons, null, 2))}</pre><h2>Operational failures and ingestion errors</h2><pre>${safe(JSON.stringify({ failures: data.operational_failures, ingestion_errors: errors }, null, 2))}</pre><h2>Task, category, and language drill-down</h2><h2>Effort and telemetry completeness</h2><pre>${safe(JSON.stringify({ effort_quality, resource_observations }, null, 2))}</pre><h2>Historical compatibility and lineage</h2><pre>${safe(JSON.stringify({ history: data.history, lineage: data.lineage }, null, 2))}</pre><h2>Metric completeness</h2><pre>${safe(JSON.stringify(configs.map(c => ({ configuration_id: c.configuration_id, metric_completeness: c.metric_completeness })), null, 2))}</pre><pre>${safe(JSON.stringify(task_category_language, null, 2))}</pre>`;
